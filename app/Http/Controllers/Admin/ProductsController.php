...
class ProductsController extends Controller
{
    public function addProduct(Request $request){
       //this code save product records in database
        if($request->isMethod('post')){
            ...
            $product->short_description = $data['short-description'];
            $product->size_guide = $data['size_guide'];
    ...
    public function editProduct(Request $request,$id=null){
        if($request->isMethod('post')){
            ...
            Product::where(['id'=>$id])->update(['shipping_info' => $data['shipping_info'],'return_info' => $data['return_info'],'stock'=> $data['stock'],'sku'=>$data['sku'],'category_id'=>$data['category_id'],
            'product_name'=>$data['product_name'],'product_code'=>$data['product_code'],
            'product_color'=>$data['product_color'], 'short_description'=> $data['short_description'],'description'=>$data['description'],'size_guide' => $data['size_guide'],
            'care'=>$data['care'],'price'=>$data['price'],'selling_price'=>$data['selling_price'],'image'=>$filename]);
...
>>>>>>> 4569e9f1ce019d06b67b51b9f9a994cf2f167a94
    ...
    public function trackOrder(Request $request ,$id=null){
         
           //$user_id = Auth::user()->id;
        $orderDetails = DB::table('orders')
            ->join('orders_products', 'orders.id', '=', 'orders_products.order_id')
            ->select('orders.id', 'orders_products.user_id','orders_products.product_name', 'orders.order_status')
            ->where('orders.id',$id)
            ->get();
